...
export interface Hint {
  id: number;
  level: "Nudge" | "Approach" | "Near Answer" | string;
  cost: number;
  content: string;
}
...
export default function LessonViewer({ lesson }: LessonViewerProps) {
  ...
  if (!lesson) {
    return <div className="p-6 text-slate-400">Loading lesson...</div>;
  }
    ...
    <div className="flex-1 flex flex-col h-full bg-[#0a0f18] text-slate-300 overflow-y-auto">
      ...
      <div className="p-6 space-y-8">
        ...
        <section className="space-y-4">
          ...
          <p className="text-slate-300 leading-relaxed text-sm">{lesson.description}</p>
        </section>

        {/* Instructions */}
        <section className="space-y-4">
          <h2 className="text-xs font-bold uppercase tracking-wider text-slate-500">Instructions</h2>
          <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800/50 text-sm leading-relaxed shadow-sm whitespace-pre-wrap">
            {lesson.instructions}
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-3">
            {lesson.hints && lesson.hints.map((hint) => {
